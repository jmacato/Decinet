...
public class WdlResampler : IResampler
{
    ...
    public void Receive(ISampleFrame? data)
    {
        ...
        if (data is not FloatSampleFrame floatFrame) return;

        if (incomingFormat.SampleRate == receivingFormat.SampleRate)
            _dspStack?.Receive(data);

        SetRates(incomingFormat.SampleRate, receivingFormat.SampleRate);


        var count = floatFrame.InterleavedSampleData.Length;
        var channels = floatFrame.ChannelCount;
