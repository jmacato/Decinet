...
namespace LibResample.Sharp
{
    ...
    internal class FilterKit
    {
            ...
            double ph, int inc)
        {
            ...
            ph *= ReSamplerX.Npc; // Npc is number of values per 1/delta in impulse
            ...
            if (inc == 1) // If doing right wing...
            {
                ...
                if (ph.Equals(0))
                {
                    hpIndex += ReSamplerX.Npc; // first sample, so we must also 
                    hdpIndex += ReSamplerX.Npc; // skip ahead in imp[] and impd[]
                }
            }
            ...
            if (interp)
            {
                while (hpIndex < endIndex)
                {
                    ...
                    hdpIndex += ReSamplerX.Npc;
                    t *= xpArray[xpIndex];
                    v += t;
                    hpIndex += ReSamplerX.Npc;
                    xpIndex += inc;
                }
            }
            else
